...
from gym.envs.mujoco.gather.embedded_viewer import EmbeddedViewer
from gym.envs.mujoco.mujoco_utils import arg
...
class GatherViewer(MjViewer):
    def __init__(self, env):
        ...
        green_ball_model = MjModel(os.path.abspath(
                os.path.join(os.path.dirname(__file__), "../assets", 'green_ball.xml')
        ))
        ...
        red_ball_model = MjModel(os.path.abspath(
                os.path.join(os.path.dirname(__file__), "../assets", 'red_ball.xml')
        ))
...
class GatherEnv(ProxyEnv):
    ...
    def __init__(
        ...
        xml_path = os.path.join(os.path.dirname(__file__), "../assets", model_cls.FILE)
        ...
        inner_env = model_cls(*args, model_path=file_path, **kwargs)
        # pylint: enable=not-callable
        ProxyEnv.__init__(self, inner_env)  # to access the inner env, do self.wrapped_env

    def _reset(self, also_wrapped=True):
        self.objects = []
        existing = set()
    ...
    def _step(self, action):
        ...
        info['outer_rew'] = 0
        if done:
            return self._get_obs(), self.dying_cost, done, info  # give a -10 rew if the robot dies
