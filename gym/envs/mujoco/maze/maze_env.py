...
class MazeEnv(ProxyEnv):
    ...
    def __init__(
        ...
        tree = ET.parse(xml_path) # parse the structure of the xml files.
        ...
        self._init_torso_x = torso_x # coordinate x
        self._init_torso_y = torso_y # coordinate y
        ...
        if self.__class__.MAZE_MAKE_CONTACTS:
            ...
            # add contact element into contact element
            for i in range(len(structure)):
                for j in range(len(structure[0])):
                    if str(structure[i][j]) == '1':
                        for geom in geoms:
        ...
        print("the temporary file path is", file_path)
        self._goal_range = self._find_goal_range()
        self._cached_segments = None

        inner_env = model_cls(*args, model_path=file_path, **kwargs)  # file to the robot specifications
    ...
    def _step(self, action):
        ...
        if minx <= x <= maxx and miny <= y <= maxy:
            ...
            info['outer_rew'] = 1  # we keep here the original one, so that the AvgReturn is directly the freq of success
